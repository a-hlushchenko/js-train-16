...
function filterStudentsByGrade(students, grade) {
  ...
  const filterArr = students.filter((value) => value.grade === grade);
  filterArr.forEach((value) => console.log(value.name));
...
function compareArrays(arr1, arr2) {
  const result1 = JSON.stringify(arr1.sort((val1, val2) => val1 - val2));
  const result2 = JSON.stringify(arr2.sort((val1, val2) => val1 - val2));
  console.assert(result1 === result2, "error");
...
function calculateTotalPrice(products) {
  console.time();
  let total = 0;
  products.forEach((value) => (total += value.price));
...
function countVowelsAndConsonants(word) {
  ...
  for (const value of word) {
    if (str1.includes(value.toLowerCase())) {
